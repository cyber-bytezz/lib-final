...
/**
 * Administrative component for managing the book inventory.
 * Provides functionality to list, create, update, and delete books.
 */
const BookManagement: React.FC = () => {
  // State for storing the list of books fetched from the database
  const [books, setBooks] = useState<Book[]>([]);
  // State to track the loading status during async operations
  const [loading, setLoading] = useState(true);
  // State for the form fields, initialized with empty values
  ...
  /**
   * Fetches all books from the database and updates the local state.
   */
  ...
  /**
   * Populates the form state with an existing book's data for editing.
   */
  const handleEdit = (b: Book) => setForm(b);

  /**
   * Triggers the deletion of a book after user confirmation.
   * @param id - The unique identifier (New NO.) of the book to delete.
   */
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-1">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm sticky top-24">
          <h3 className="text-lg font-bold text-slate-900 mb-4">
            {form["New NO."] ? 'Edit Book' : 'Add New Book'}
          </h3>
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex gap-2">
              <button type="submit" className="flex-1 bg-indigo-600 text-white font-bold py-2 rounded-lg hover:bg-indigo-700 transition-colors">
                Save
              </button>
              {/* Reset button only visible during edit mode */}
              {form["New NO."] && (
                <button 
                  type="button" 
                  onClick={() => setForm({ "S.NO": 0, "New NO.": "", "NAME OF THE BOOK": "", "AUTHOR NAME": "", "PUBLICATION": "" })} 
      ...
      {/* Right Column: Inventory List Section */}
      <div className="lg:col-span-2">
        {loading ? <Loader /> : (
          <div className="space-y-4">
            ...
            {/* Quick-action delete buttons for each book */}
            <div className="mt-4 flex flex-wrap gap-2">
              {books.map(b => (
                <button 
                  key={b["New NO."]} 
                  onClick={() => handleDelete(b["New NO."])} 
